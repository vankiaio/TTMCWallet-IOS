...
#define JS_INTERACTION_METHOD_getTtmcAccount @"getTtmcAccount"
#define JS_INTERACTION_METHOD_getWalletWithAccount @"getWalletWithAccount"
#define JS_INTERACTION_METHOD_getTtmcBalance @"getTtmcBalance"
#define JS_INTERACTION_METHOD_getTtmcAccountInfo @"getTtmcAccountInfo"
...
- (WKWebView *)webView{
    if (!_webView) {
        ...
        if (@available(iOS 9.0, *)) {
            self.webView.customUserAgent = @"PocketTtmcIos";
        } else {
            // Fallback on earlier versions
        }
        self.allowZoom = YES;
...
-(void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    self.allowZoom = NO;
    [self passTtmcAccountNameToJS];
    [self passWalletInfoToJS];
    WS(weakSelf);
    // 确保js 能收到 ttmcAccountName
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [weakSelf passTtmcAccountNameToJS];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([self.WKScriptMessageName isEqualToString:JS_INTERACTION_METHOD_PUSHACTION] || [self.WKScriptMessageName isEqualToString:JS_INTERACTION_METHOD_PUSH]) {
        ...
        if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getTtmcAccount]) {
            [self js_pushMessage_method_getTtmcAccount];
        ...
        }else if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getTtmcBalance]){
            [self js_pushMessage_method_getTtmcBalance];
        }else if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getTtmcAccountInfo]){
            [self js_pushMessage_method_getTtmcAccountInfo];
...
- (void)passTtmcAccountNameToJS{
    NSString *js = [NSString stringWithFormat:@"getTtmcAccount('%@')", self.choosedAccountName];
...
- (void)js_pushMessage_method_getTtmcAccount{
    WS(weakSelf);
    [self.dappDetailService getTtmcAccount:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}
...
- (void)js_pushMessage_method_getTtmcBalance{
    WS(weakSelf);
    GetTtmcBalanceModel *model = [GetTtmcBalanceModel mj_objectWithKeyValues:self.dappPushMessageModel.params];
    ...
    [self.dappDetailService getTtmcBalance:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}

- (void)js_pushMessage_method_getTtmcAccountInfo{
    WS(weakSelf);
    [self.dappDetailService getTtmcAccountInfo:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}
...
- (void)dappChangeAccountOnNavigationRightViewDidClick{
    ...
    builder.cancelText = NSLocalizedString(@"选择您的TTMC账号", nil);
    WS(weakSelf);
...
- (void)loadWebView{
    ...
    if ([self.model.dappUrl isEqualToString:@"https://ttmcflare.io"]) {
