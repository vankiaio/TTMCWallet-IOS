...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell setAssestsSwitchStatusDidChangeBlock:^(RecommandToken *token, UISwitch *assestsSwitch) {
        ...
        if ([token.assetName isEqualToString:@"OCT"] || [token.assetName isEqualToString:@"TTMC"]) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.assetName isEqualToString:@"OCT"] || [model.assetName isEqualToString:@"TTMC"]) {
        [TOASTVIEW showWithText: NSLocalizedString(@"该资产无法取消关注", nil)];
    }
}
