...
- (void)configHeaderView{
    ...
    [self.getSparklinesRequest getDataSusscess:^(id DAO, id data) {
        ...
        if ([weakSelf.model.token_symbol isEqualToString:@"TTMC"] && [weakSelf.model.contract_name isEqualToString:ContractName_TTMCIOTOKEN]) {
            tendencyUrlStr = data[@"data"][@"sparkline_ttmc_png"];
...
- (void)SocialSharePanelViewDidTap:(UITapGestureRecognizer *)sender{
    ...
    if ([self.model.token_symbol isEqualToString:@"TTMC"]) {
        model.imageName = @"ttmc_avatar";
    ...
    model.webPageUrl = @"https://pocketttmc.com";
    NSLog(@"%@", platformName);
...
- (void)assestsDetailFooterViewDidClick:(UIButton *)sender{
    if (sender.tag == 1000) {
        ...
        if ([self.model.token_symbol isEqualToString:@"OCT"]  || [self.model.token_symbol isEqualToString:@"TTMC"]) {
