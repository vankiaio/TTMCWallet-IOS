...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"back" title:NSLocalizedString(@"锁仓记录", nil)rightBtnImgName:@"" delegate:self];
    ...
    [self.mainService buildDataSource:^(NSNumber *dataCount, BOOL isSuccess) {
        if (isSuccess) {
            ...
            if ([dataCount isEqualToNumber:@0]) {
                ...
                self.headerView.lockedAmountLabel.text = [NSString stringWithFormat:@"%@: %@ TTMC",NSLocalizedString(@"锁仓总额", nil), self.mainService.assestLocksResult.amountlocked];
            }
        }else{
            [weakSelf.mainTableView.mj_header endRefreshing];
            [weakSelf.mainTableView.mj_footer endRefreshing];
