...
- (void)SocialSharePanelViewDidTap:(UITapGestureRecognizer *)sender{
    if (self.authRedPacketResult.data.payStatus.integerValue == 1) {////支付成功 可以进行红包的发送
        ...
        // http://static.pocketttmc.top:8003
        model.webPageUrl = [NSString stringWithFormat:@"%@:8003?id=%@&verifystring=%@", REQUEST_HTTP_STATIC_BASEURL,self.redPacketModel.redPacket_id,self.authRedPacketResult.data.verifyString];
