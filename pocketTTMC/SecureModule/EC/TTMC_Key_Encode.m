...
@implementation TTMC_Key_Encode
...
+ (NSString *)ttmc_publicKey_with_wif:(NSString *)wif{
    ...
    NSString *ttmcPubKey = [NSString stringWithFormat:@"TTMC%@", [NSString stringWithUTF8String:base]];
    return ttmcPubKey;
}
...
+ (NSString *)encode_ttmc_PublicKey_with_uecc_publicKey_bytes_data:(NSData *)uecc_publicKey_bytes_data{
    ...
    return [NSString stringWithFormat:@"TTMC%@", [NSString stringWithUTF8String:base]];;
}
...
+ (NSData *)decode_ttmc_publicKey:(NSString *)ttmc_publicKey{
    if (!(ttmc_publicKey.length > 0)) {
        NSLog(@"parameter ttmc_publicKey can't be nil!");
        return nil;
    }
    if (![ttmc_publicKey hasPrefix:@"TTMC"]) {
        NSLog(@"parameter ttmc_publicKey has not prefix 'TTMC'!");
        return nil;
    }
    const char *b58 = [[ttmc_publicKey substringFromIndex:3] UTF8String];
    unsigned char bin[100];
    size_t binlen = 37;
    unsigned char *hash;
    ...
    for(int i=0;i<4;i++){
        if(validateHash[i]!=hash[i]){
            NSLog(@"parameter ttmc_publicKey validate failed!");
            return nil;
        }
    }
