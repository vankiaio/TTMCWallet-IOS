...
+ (NSData *)getBytesForSignature:(NSData *)chainId andParams:(NSDictionary *)paramsDic andCapacity:(int)capacity{
    TtmcByteWriter *writer = [[TtmcByteWriter alloc] initWithCapacity:capacity];
    if (chainId) {
    ...
    //transaction_extensions :: add transaction_extensions in transaction, although it dttmc not support yet.
    [writer putVariableUInt:0];
