...
@property(nonatomic , strong) BackupTTMCAccountService *backupTTMCAccountService;
...
- (BackupTTMCAccountService *)backupTTMCAccountService{
    if (!_backupTTMCAccountService) {
        _backupTTMCAccountService = [[BackupTTMCAccountService alloc] init];
    }
    return _backupTTMCAccountService;
}
...
- (void)validateInputFormat{
    // 验证私钥格式是否正确
    private_Key_1_is_validate = [TTMC_Key_Encode validateWif:self.headerView.textView1.text];
    private_Key_2_is_validate = [TTMC_Key_Encode validateWif:self.headerView.textView2.text];
...
- (void)createPublicKeys{
    // 将用户导入的私钥生成公钥
    public_key_1_from_local = [TTMC_Key_Encode ttmc_publicKey_with_wif:self.headerView.textView1.text];
    public_key_2_from_local = [TTMC_Key_Encode ttmc_publicKey_with_wif:self.headerView.textView2.text];
...
- (void)saveAccountWithAccountInfo:(AccountInfo *)accountInfo{
    ...
    self.backupTTMCAccountService.backupTtmcAccountRequest.uid = CURRENT_WALLET_UID;
    self.backupTTMCAccountService.backupTtmcAccountRequest.ttmcAccountName = accountInfo.account_name;
    [self.backupTTMCAccountService backupAccount:^(id service, BOOL isSuccess) {
        if (isSuccess) {
            NSLog(@"备份到服务器成功!");
        }
    }];
