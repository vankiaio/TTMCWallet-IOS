...
@property(nonatomic , strong) TtmcPrivateKey *ownerPrivateKey;
@property(nonatomic , strong) TtmcPrivateKey *activePrivateKey;
...
- (void)createAllKeys{
    self.ownerPrivateKey = [[TtmcPrivateKey alloc] initTtmcPrivateKey];
    self.activePrivateKey = [[TtmcPrivateKey alloc] initTtmcPrivateKey];
    NSLog(@"ownerPrivateKey:\n %@ \n ownerPublicKey:\n %@\n activePrivateKey:\n%@\n activePublicKey:\n %@\n", self.ownerPrivateKey.ttmcPrivateKey , self.ownerPrivateKey.ttmcPublicKey , self.activePrivateKey.ttmcPrivateKey, self.activePrivateKey.ttmcPublicKey);
}
...
- (void)configCreateAccountOrderRequestParams{
    ...
    if (self.headerView.privateKeyBeSameModeBtn.selected == YES) {
        self.payRegistAccountService.createAccountOrderRequest.ownerKey = self.ownerPrivateKey.ttmcPublicKey;
        self.payRegistAccountService.createAccountOrderRequest.activeKey = self.ownerPrivateKey.ttmcPublicKey;
    }else{
        self.payRegistAccountService.createAccountOrderRequest.ownerKey = self.ownerPrivateKey.ttmcPublicKey;
        self.payRegistAccountService.createAccountOrderRequest.activeKey = self.activePrivateKey.ttmcPublicKey;
    }
}
...
- (void)storeAccountInfoToLocalDatabase{
    ...
    if (self.headerView.privateKeyBeSameModeBtn.selected == YES) {
        model.account_owner_public_key = self.ownerPrivateKey.ttmcPublicKey;
        model.account_active_public_key = self.ownerPrivateKey.ttmcPublicKey;
        model.account_owner_private_key = [AESCrypt encrypt:self.ownerPrivateKey.ttmcPrivateKey password:self.password];
        model.account_active_private_key = [AESCrypt encrypt:self.ownerPrivateKey.ttmcPrivateKey password:self.password];
    }else{
        model.account_owner_public_key = self.ownerPrivateKey.ttmcPublicKey;
        model.account_active_public_key = self.activePrivateKey.ttmcPublicKey;
        model.account_owner_private_key = [AESCrypt encrypt:self.ownerPrivateKey.ttmcPrivateKey password:self.password];
        model.account_active_private_key = [AESCrypt encrypt:self.activePrivateKey.ttmcPrivateKey password:self.password];
    }
