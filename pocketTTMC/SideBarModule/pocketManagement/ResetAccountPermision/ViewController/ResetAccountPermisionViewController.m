...
@property(nonatomic , strong) TtmcPrivateKey *privateKey;
@end
...
- (TtmcPrivateKey *)privateKey{
    if (!_privateKey) {
        _privateKey = [[TtmcPrivateKey alloc] initTtmcPrivateKey];
    }
    return _privateKey;
}
...
- (void)validateInputFormat{
    // 验证私钥格式是否正确
    private_Key_is_validate = [TTMC_Key_Encode validateWif:self.headerView.textView.text ];
    // 将用户导入的私钥生成公钥
    public_key_from_local = [TTMC_Key_Encode ttmc_publicKey_with_wif:self.headerView.textView.text];
...
- (void)pushActicon:(NSDictionary *)argsDict{
    self.abi_json_to_binRequest.code = ContractName_TTMCIO;
    self.mainService.code = ContractName_TTMCIO;
...
- (void)addGeneratePrivateKeyView{
    ...
    self.generatePrivateKeyView.contentTextView.text = self.privateKey.ttmcPrivateKey;
