...
- (void)buildDataSource{
    ...
    [self.getCurrentRamPriceRequest getDataSusscess:^(id DAO, id data) {
        ...
        if (result.data.count > 0) {
            ...
            weakSelf.headerView.nowPriceLanel.text = [NSString stringWithFormat:@"%.4fTTMC/KB", model.price.doubleValue];
        }
...
- (void)ramManageHeaderViewConfirmStakeBtnDidClick{
    ...
    if (IsStrEmpty(self.headerView.ttmcAmountTF.text)) {
    ...
    if (self.headerView.ramManageHeaderViewCurrentAction == RamManageHeaderViewCurrentActionBuyRam) {
        if (self.headerView.ttmcAmountTF.text.doubleValue > self.ttmcResourceResult.data.core_liquid_balance.doubleValue ) {
...
- (void)buyRam{
    ...
    self.buy_ram_abi_json_to_bin_request.quant = [NSString stringWithFormat:@"%.4f %@",self.headerView.ttmcAmountTF.text.doubleValue, SymbolName_TTMC];
    WS(weakSelf);
...
- (void)sellRam{
    ...
    double bytes;
    bytes = self.headerView.ttmcAmountTF.text.doubleValue * 1024;
...
- (void)pushTransactionDidFinish:(TTMCResourceResult *)result{
    ...
    [self removeLoginPasswordView];
    [self loadNewData];
    self.headerView.ttmcAmountTF.text = nil;
}
