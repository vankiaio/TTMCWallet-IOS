...
- (void)viewDidLoad {
    ...
    self.headerView.ttmcAccountNameLabel.text = CURRENT_ACCOUNT_NAME;
}
...
//    let data = timestamp + account + uuID + ref     //ref为钱包名，标示来源
    ...
    const int8_t *private_key = [[TTMC_Key_Encode getRandomBytesDataWithWif:[AESCrypt decrypt:accountInfo.account_active_private_key password:self.loginPasswordView.inputPasswordTF.text]] bytes];
...
- (void)sendSignedResultToDappServer{
    ...
    [params setObject:VALIDATE_STRING(@"PocketTtmc") forKey:@"ref"];
